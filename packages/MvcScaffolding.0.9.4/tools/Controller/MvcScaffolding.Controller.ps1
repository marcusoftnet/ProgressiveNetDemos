...
if ($templateFile) {
	...
	if (!$NoChildItems) {
		...
		Scaffold Views -Controller $controllerNameWithoutSuffix -ModelType $foundModelType.FullName -Area $Area -Layout $Layout -SectionNames $SectionNames -PrimarySectionName $PrimarySectionName -Project $Project -CodeLanguage $CodeLanguage -Force:$Force
	}
}